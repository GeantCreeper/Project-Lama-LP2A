<h1 align="center">Mention F</h1>

<p align="center">
  <em>Le jeu de cartes où rater son semestre demande de la stratégie.</em>
  <br>
  <em>Il s'agit d'un jeu basé sur Lama mais comprenant des modifications afin d'adopter des visuels en rapport avec l'UTBM.</em>
</p>

<br>

## Règles du jeu

### But du jeu
L'objectif est de vous débarrasser de toutes vos cartes pour gagner des crédits ECTS. Le joueur avec le plus de crédits à la fin de la partie valide son année.

### Le Matériel
* **[Nombre]** cartes U.E. (Unités d'Enseignement), numérotées de 1 à 6.
* **[Nombre]** cartes Jury.
* **[Nombre]** jetons ECTS.

### Déroulement d'un tour
À votre tour, vous devez effectuer **une seule** des trois actions suivantes :
1. **Jouer une carte :** Poser une carte de même valeur ou d'une valeur supérieure de 1 par rapport au sommet de la pile. Sur un 6, vous pouvez poser un 6 ou une carte Jury. Sur une carte Jury, vous pouvez poser un Jury ou un 1.
2. **Piocher une carte :** Prendre la première carte de la pioche.
3. **Abandonner le semestre :** Quitter la manche en cours. Vous conservez vos cartes en main et ne jouez plus jusqu'à la fin de la manche.

### Fin de la manche
La manche se termine quand un joueur vide sa main ou quand tous les joueurs ont abandonné. Les cartes restantes en main valent des points de pénalité :
* Les cartes numérotées valent leur valeur faciale (une seule fois par valeur).
* La carte Jury vaut 10 points de pénalité.

---

## Images du jeu

*[lien image du menu]*

![Placeholder - Carte Jury](chemin)
*Légende : Maquette de la carte Jury.*

![Placeholder - Plateau de jeu](chemin)
*Légende : Aperçu d'une partie en cours.*

---

## Architecture et UML

Voici le diagramme de classes modélisant le moteur du jeu.

![Diagramme UML](my_uml.png)
